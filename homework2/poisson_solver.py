...
def main():
    ...
    for Ni, k_max_i in zip(N, k_max):
        ...
        if method == 'jacobi':
            T, k = jacobi_method(Ni, x, y, h, Q, T_initial, k_max_i, tolerance, verbose=verbose)
        elif method == 'gauss-seidel':
            if ghost_cells:
                raise NotImplementedError("Gauss-Seidel method with ghost cells is not implemented.")
            T, k = gauss_seidel_method(Ni, x, y, h, Q, T_initial, k_max_i, tolerance, verbose=verbose)
        elif method == 'sor':
            if ghost_cells:
                raise NotImplementedError("SOR method with ghost cells is not implemented.")
            T, k = sor_method(Ni, x, y, h, Q, T_initial, k_max_i, alpha, tolerance, verbose=verbose)
        end_time = time.time()
        ...
        if output_folder:
            plot_solution(x, y, T, method.capitalize(), output_file=f'{output_folder}/{method}_{Ni}.png')
            plot_solution(x, y, T_analytical, 'Analytical', output_file=f'{output_folder}/analytical_{Ni}.png')
